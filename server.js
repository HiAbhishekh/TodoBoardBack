// without namigng conv. old db updated with priorities..correct
/*
...
const addCompletedColumnIfNotExists = async () => {
  try {
    const [columns] = await pool.query('SHOW COLUMNS FROM kanban_items LIKE "completed"');
    if (columns.length === 0) {
      await pool.query('ALTER TABLE kanban_items ADD COLUMN completed BOOLEAN DEFAULT false');
      console.log('Added completed column to kanban_items table');
    }
  } catch (error) {
    console.error('Error checking/adding completed column:', error);
  }
...
const addPriorityColumnIfNotExists = async () => {
  try {
    const [columns] = await pool.query('SHOW COLUMNS FROM kanban_items LIKE "priority"');
    if (columns.length === 0) {
      await pool.query('ALTER TABLE kanban_items ADD COLUMN priority ENUM("low", "medium", "high") DEFAULT "low"');
      console.log('Added priority column to kanban_items table');
    }
  } catch (error) {
    console.error('Error checking/adding priority column:', error);
  }
...
const createDefaultBoardIfNotExists = async () => {
  try {
    const [existingDefaultBoard] = await pool.query('SELECT * FROM kanban_boards WHERE title = "MyBoard"');
    if (existingDefaultBoard.length === 0) {
      await pool.query('INSERT INTO kanban_boards (title) VALUES (?)', ['MyBoard']);
      console.log('Default board "MyBoard" created.');
    } else {
      console.log('Default board "MyBoard" already exists.');
    }
...
// Get all boards
app.get('/api/kanban_boards', async (req, res) => {
  try {
    const [boards] = await pool.query('SELECT * FROM kanban_boards ORDER BY created_at DESC');
    
    for (let board of boards) {
      const [cards] = await pool.query('SELECT * FROM kanban_cards WHERE board_id = ?', [board.id]);
      
      for (let card of cards) {
        const [items] = await pool.query(
          'SELECT *, DATE_FORMAT(created_time, "%Y-%m-%dT%H:%i:%sZ") AS createdTime FROM kanban_items WHERE card_id = ?',
          [card.id]
        );
        card.items = items;
      }
...
// Create new board
app.post('/api/kanban_boards', async (req, res) => {
  try {
    const { title } = req.body;
    const [result] = await pool.query('INSERT INTO kanban_boards (title) VALUES (?)', [title]);
    const [board] = await pool.query('SELECT * FROM kanban_boards WHERE id = ?', [result.insertId]);
    res.status(201).json(board[0]);
  } catch (error) {
    console.error('Error creating board:', error);
    res.status(500).json({ error: 'Internal server error' });
...
// Delete board
app.delete('/api/kanban_boards/:id', async (req, res) => {
  try {
...
// Create new card
app.post('/api/kanban_boards/:boardId/kanban_cards', async (req, res) => {
  try {
    const { title, color } = req.body;
    const { boardId } = req.params;
    
    const [result] = await pool.query(
      'INSERT INTO kanban_cards (board_id, title, color) VALUES (?, ?, ?)',
      [boardId, title, color || '#ffffff']
    );
    
    const [card] = await pool.query('SELECT * FROM kanban_cards WHERE id = ?', [result.insertId]);
    res.status(201).json(card[0]);
  } catch (error) {
    console.error('Error creating card:', error);
    res.status(500).json({ error: 'Internal server error' });
...
// Update card color
app.patch('/api/kanban_cards/:id/color', async (req, res) => {
  try {
    const { color } = req.body;
    const { id } = req.params;
    
    await pool.query('UPDATE kanban_cards SET color = ? WHERE id = ?', [color, id]);
    const [card] = await pool.query('SELECT * FROM kanban_cards WHERE id = ?', [id]);
    res.json(card[0]);
  } catch (error) {
    console.error('Error updating card color:', error);
    res.status(500).json({ error: 'Internal server error' });
...
// Delete card
app.delete('/api/kanban_cards/:id', async (req, res) => {
  try {
    await pool.query('DELETE FROM kanban_cards WHERE id = ?', [req.params.id]);
    res.status(204).send();
  } catch (error) {
    console.error('Error deleting card:', error);
    res.status(500).json({ error: 'Internal server error' });
...
// Create new item
app.post('/api/kanban_cards/:cardId/kanban_items', async (req, res) => {
  try {
    const { title, priority = 'low' } = req.body; // Default priority to 'low'
    const { cardId } = req.params;
    
    const [result] = await pool.query(
      'INSERT INTO kanban_items (card_id, title, created_time, completed, priority) VALUES (?, ?, NOW(), false, ?)',
      [cardId, title, priority]
    );

    const [item] = await pool.query(
      'SELECT id, title, card_id, completed, priority, DATE_FORMAT(created_time, "%Y-%m-%dT%H:%i:%sZ") AS createdTime FROM kanban_items WHERE id = ?',
      [result.insertId]
    );
...
app.patch('/api/kanban_items/:itemId', async (req, res) => {
  try {
    const { itemId } = req.params;
    const { completed, priority } = req.body;
    ...
    await pool.query(
      `UPDATE kanban_items SET ${updateFields.join(', ')} WHERE id = ?`,
      updateValues
    );
...
app.get('/api/kanban_cards/:cardId/kanban_items', async (req, res) => {
  const { cardId } = req.params;
  const limit = parseInt(req.query.limit) || 10;
  const offset = parseInt(req.query.offset) || 0;
  ...
  try {
    const [items] = await pool.query(
      'SELECT id, title, card_id, completed, priority, DATE_FORMAT(created_time, "%Y-%m-%dT%H:%i:%sZ") AS createdTime FROM kanban_items WHERE card_id = ? LIMIT ? OFFSET ?',
      [cardId, limit, offset]
    );
...
// Move item to a different card
app.patch('/api/kanban_items/:itemId/move', async (req, res) => {
  try {
    const { itemId } = req.params;
    const { targetCardId } = req.body;
    ...
    // Update the item's card_id in the database
    await pool.query('UPDATE kanban_items SET card_id = ? WHERE id = ?', [targetCardId, itemId]);

    // Fetch the updated item to return to the client
    const [updatedItem] = await pool.query('SELECT * FROM kanban_items WHERE id = ?', [itemId]);
...
// Delete item
app.delete('/api/kanban_items/:id', async (req, res) => {
  try {
